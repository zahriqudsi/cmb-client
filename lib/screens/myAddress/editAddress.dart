...
class EditAddress extends StatefulWidget {
  ...
  AddresssBookItemModel addressItem;

  EditAddress({Key? key, required this.authToken, required this.addressItem})
      : super(key: key);

  @override
  _EditAddressState createState() =>
      _EditAddressState(authToken: authToken, addressItem: addressItem);
}
...
class _EditAddressState extends State<EditAddress> {
  ...
  _EditAddressState({required this.authToken, required this.addressItem});
  ...
  void initState() {
    ...
    myHome.text = addressItem.label;
    fname.text = addressItem.firstName;
    lname.text = addressItem.lastName;
    phone.text = addressItem.phone;
    email.text = addressItem.email;
    address.text = addressItem.address;
    postCode.text = addressItem.zipCode;

    getCountries();
    getStates(addressItem.countryId.toString());
    getCities(addressItem.stateId.toString());

    setState(() {
      countryValue = addressItem.countryId.toString();
      stateValue = addressItem.stateId.toString();
      cityValue = addressItem.cityId.toString();
    });
  }
  ...
  void udpateAddress() async {
    try {
      setState(() {
        isLoading = true;
      });

      bool status = await appRepo.updateAddress(addressItem.id, {
        "label": myHome.text,
