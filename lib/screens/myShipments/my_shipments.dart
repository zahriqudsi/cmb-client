...
class MyShipments extends StatefulWidget {
  ...
  const MyShipments({Key? key, required this.authToken}) : super(key: key);

  @override
  _MyShipmentsState createState() => _MyShipmentsState(authToken);
}
...
class _MyShipmentsState extends State<MyShipments> {
  ...
  _MyShipmentsState(TokenModel authTokenModel) {
    appRepo = AppRepository(authToken: authTokenModel.token);
    authToken = authTokenModel;
  }

  late MyShipmentsModel shipments;
  ...
  void initState() {
    ...
    getShipments(shipemntsPage);
  }

  getShipments(int shipmentsPage) async {
    setState(() {
      isLoading = true;
    });

    try {
      MyShipmentsModel response = await appRepo.myShipments(shipemntsPage);

      setState(() {
        print(response);
        shipments = response;
        information = shipments.details as List<ShipmentInformation>;
        isLoading = false;
      });
    } catch (e) {
      print(e);
  ...
  Widget ShipmentItems(
      ShipmentInformation packageHistories, TokenModel authToken) {
    // final DateFormat formatter = DateFormat('dd MMMM H:m');
        ...
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (context) => ShipmentDetails(
              authToken: authToken,
              shipmentId: packageHistories.id,
            ),
          ),
        );
      },
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Text(
                  "# " + packageHistories.trackingCode.toString(),
            ...
            Container(
              child: Text(
                "Package - " +
                    (packageHistories.length.toString()) +
                    "ft " +
                    (packageHistories.width.toString()) +
                    "ft " +
                    (packageHistories.height.toString()) +
                    "ft " +
                    (packageHistories.weight.toString()) +
                    "Kg ",
                style: const TextStyle(
                  fontSize: 20,
                  fontFamily: 'Roboto',
